...
const CONFIG = {
    ...
    scraperTimeout: 20000, // Boosted timeout for heavy pages
};
...
const server = http.createServer(async (req, res) => {
    ...
    try {
        if (pathname === '/') {
            ...
            res.end(JSON.stringify({ status: 'ok', service: 'xstream-api-real', version: '3.0.0' }));
            return;
        }
        ...
        if (pathname === '/api/videos') {
            ...
            if (action === 'home') {
                try {
                    ...
                    console.error(`[XVideos] Home scrape failed: ${err.message}`);
        ...
        // THE FIX: Bulletproof Video Data Extraction
        if (pathname === '/api/video-data') {
            ...
            try {
                // Construct the correct XVideos URL whether it's numeric or alphanumeric
                const cleanId = id.replace(/[^0-9a-zA-Z_-]/g, ''); // Strip bad characters
                ...
                console.log(`[XVideos] Fetching video source: ${videoUrl}`);
                const html = await fetchHtml(videoUrl);

                // Aggressively hunt for video links using multiple regex strategies
                ...
                // Fallback: If they change variable names, just find ANY mp4/m3u8 inside a script tag
                ...
                // Route exactly what the frontend needs
                let bestStream = '';
                ...
                else bestHls = bestStream; // Always give the player *something* to play

                if (!bestStream && !bestHls) {
                    throw new Error("No video URLs found. ID format might be wrong or site blocked.");
                }
                ...
                console.error(`[XVideos] Error extracting video data: ${err.message}`);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                // Sending back an empty error string tells frontend to show error natively
        ...
        // Proxy for video streaming (Bypasses IP locks)
        if (pathname === '/api/stream') {
            ...
            // Preserve range headers for seeking in the frontend player
            if (req.headers.range) {
                fetchHeaders['Range'] = req.headers.range;
            }

            client.get(decodedUrl, { headers: fetchHeaders }, (streamRes) => {
                ...
                // MP4 direct piping with Range support
...
server.listen(CONFIG.port, CONFIG.host, () => {
    console.log(`🚀 Perfected XStream API Server started on http://${CONFIG.host}:${CONFIG.port}`);
});
